...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ttd { margin-top: 36px; display: table; width: 100%; }
        .ttd-left { display: table-cell; width: 50%; text-align: center; }
        .ttd-right { display: table-cell; width: 50%; text-align: center; }
        .ttd-inner p { font-size: 11px; margin-bottom: 52px; }
        .ttd-inner .nama { font-weight: bold; border-top: 1px solid #333; padding-top: 4px; font-size: 11px; display: inline-block; min-width: 160px; }
...
<body>
    ...
    <div class="no-print">
        <a href="{{ route('guru.aspirasi.detail', $aspirasi->id_aspirasi) }}" class="btn-back">&#8592; Kembali</a>
    ...
    <div class="section">
        ...
        <div class="section-body">
            <table class="detail">
                <tr><td>Nomor Aspirasi</td><td>:</td><td>#{{ str_pad($aspirasi->id_aspirasi, 4, '0', STR_PAD_LEFT) }}</td></tr>
                <tr><td>Kategori</td><td>:</td><td>{{ $aspirasi->kategori->nama_kategori ?? '-' }}</td></tr>
                <tr><td>Ruangan / Lokasi</td><td>:</td><td>{{ $aspirasi->ruangan->nama_ruangan ?? $aspirasi->lokasi }}</td></tr>
                <tr><td>Tanggal Pengiriman</td><td>:</td><td>{{ $aspirasi->created_at->format('d/m/Y H:i') }} WIB</td></tr>
                <tr><td>Tanggal Selesai</td><td>:</td><td>{{ $aspirasi->updated_at->format('d/m/Y H:i') }} WIB</td></tr>
                <tr><td>Status</td><td>:</td><td style="color:#198754; font-weight:bold;">&#10004; Selesai</td></tr>
            </table>
        </div>
    </div>

    <div class="section">
        <div class="section-header">Informasi Pelapor</div>
        <div class="section-body">
            <table class="detail">
                <tr>
                    <td>Nama</td><td>:</td>
                    <td>{{ $aspirasi->user->siswa->nama ?? $aspirasi->user->guru->nama ?? $aspirasi->user->name }}</td>
                </tr>
                <tr>
                    <td>Email</td><td>:</td>
    ...
    <div class="ttd">
        <div class="ttd-left">
            <div class="ttd-inner">
                <p>Pelapor,<br>{{ now()->format('d/m/Y') }}</p>
                <span class="nama">{{ $aspirasi->user->siswa->nama ?? $aspirasi->user->guru->nama ?? $aspirasi->user->name }}</span>
            </div>
        </div>
        <div class="ttd-right">
            <div class="ttd-inner">
